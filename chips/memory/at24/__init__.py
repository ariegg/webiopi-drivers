...
#   1.5    2015-09-09    Bugfix for range() overflow error (and save space).
#   1.6    2015-11-04    Added REST mapping for multiple bytes writing.
#   1.7    2016-08-10    Modified REST mapping for multiple bytes writing.
#                        Changed most path parameter types from %s to %d.
#   1.8    2016-08-11    Added @api annotations.
#
#   Usage remarks
#
...
#---------- Memory abstraction REST implementation ----------
    ...
    @response(contentType=M_JSON)
    def readSomeMemoryBytes(self, bounds):
    ...
    @api("Memory", 3)
    @request("POST", "memory/bytes/series/%(valueSeries)s")
    @response(contentType=M_JSON)
    def writeSomeMemoryBytes(self, valueSeries):
        vs = valueSeries.split(",")
        start = toint(vs[0])
        values = []
        for val in vs[1:]:
            values.append(toint(val))
        self.writeMemoryBytes(start, values)
        return self.readSomeMemoryBytes(("%s,%s" % (start, start + len(values))))
    ...
    @api("Memory", 3)
    @request("GET", "memory/bit/%(address)d")
    @response("%d")
    ...
    @api("Memory", 3)
    @request("POST", "memory/bit/%(address)d/%(value)d")
    @response("%d")
    ...
    @api("Memory")
    @request("GET", "memory/byte/%(address)d")
    @response("0x%02X")
    ...
    @api("Memory")
    @request("POST", "memory/byte/%(address)d/%(value)d")
    @response("0x%02X")
    ...
    @api("Memory", 3)
    @request("GET", "memory/word/%(address)d")
    @response("0x%04X")
    ...
    @api("Memory", 3)
    @request("POST", "memory/word/%(address)d/%(value)d")
    @response("0x%04X")
    ...
    @api("Memory", 3)
    @request("GET", "memory/long/%(address)d")
    @response("0x%08X")
    ...
    @api("Memory", 3)
    @request("POST", "memory/long/%(address)d/%(value)d")
    @response("0x%08X")
