...
#   0.91   2016-02-01    Some Optimazations.
#
#   Config parameters
#
...
#---------- Class initialisation ----------
    ...
    def __init__(self, chip, vref, channelCount, resolution, name, chipsCount):
        ...
        # Calculate padding
        unpaddedNumberOfBits = (self.ADDRESS_BITS + 8) * self.chips
        debug("%s - Unpadlength=%s" % (self.name, unpaddedNumberOfBits))

        lastBit = (unpaddedNumberOfBits % 8)
        if lastBit > 0:
            padLength = 8 - lastBit
        else:
            padLength = 0
        ...
        self.padString = "".rjust(padLength,'0')
        debug("%s - Padding=\"%s\"" % (self.name, self.padString))
...
#---------- DAC abstraction related methods ----------
    ...
    def __analogWrite__(self, channel, value):
        ...
        portAddr = channel%self.slice
        addressString = (bin(portAddr)[2:]).rjust(self.ADDRESS_BITS,'0')
        debug("%s - Address string=\"%s\"" % (self.name, addressString))

        slotValues = self.values[portAddr::self.slice]
        slotValues.reverse()
        ...
        bitSequence = self.padString + bitSequence
        debug("%s - Bitsequence=%s" % (self.name, bitSequence))
